...
function adopt_soft_properties(element,data)
{
	...
	if(element.type=="monster" && G.monsters[element.mtype])
	{
		...
		["speed","xp","attack","frequency","rage","aggro","armor","resistance","damage_type","respawn","range","name","abilities","evasion","reflection","dreturn","immune","cooperative","spawns","special","1hp","lifesteal"].forEach(function(attr){mapping.push([attr,attr])});
...
function update_overlays()
{
	...
	if(events.abtesting)
	{
		...
		var seconds=-ssince(events.abtesting.end),minutes=parseInt(seconds/60),seconds=parseInt(seconds)%60;
...
function init_socket(args)
{
	...
	socket.on("game_response",function(data){
		...
		draw_trigger(function(){
			...
			else if(response=="upgrade_offerring_success")
			{
				resolve_deferred("upgrade",{offering:true});
			}
			...
			else if(response=="upgrade_chance" || response=="compound_chance")
			{
				...
				if(response=="upgrade_chance") resolve_deferred("upgrade",{calculate:true,chance:data.chance});
				if(response=="compound_chance") resolve_deferred("compound",{calculate:true,chance:data.chance});
			}
			...
			else if(in_arr(response,["donate_thx","donate_gum","donate_low"]))
			{
				...
				else if(response=="donate_gum") message=data.gold+"? "+data.gold+"? "+data.gold+"?! Here, take this!";
				else if(response=="donate_low") message="They say there's no small contribution.. BUT THEY ARE OBVIOUSLY WRONG. "+data.gold+"??!!! GET LOST";
	...
	socket.on('action',function(data){
		...
		if(new_attacks)
		{
			var manim=null,sanim=null;
			var owner=attacker,entity=target,anim=data.anim;

			if(data.projectile)
				map_animation(G.projectiles[data.projectile].animation,{x:get_x(owner),y:get_y(owner)-15,target:entity,m:data.m,id:data.pid,filter:!G.projectiles[data.projectile].pure&&owner&&owner.filter_wglow});
		...
		else
		{
			...
			else if(0 && evade) d_line(owner,entity,{color:"evade"});
			else if(data.reflect) d_line(owner,entity,{color:"reflect"});
			else if(owner && owner.skin=="konami") d_line(owner,entity,{color:random_one(["#63388F","#D1B416","#CF3327","#2D82D2"])});
			else if(data.anim=="taunt") d_line(owner,entity,{color:"taunt"});
			else if(data.anim=="poisonarrow") d_line(owner,entity,{color:colors.poison,size:2}),color=colors.poison;
			else if(data.anim=="mentalburst")
			{
				d_line(owner,entity,{color:colors.mp,size:2});
				if(data.kill) color=colors.mp;
			}
			else if(data.anim=="burst") d_line(owner,entity,{color:"burst"}),color="burst";
			else if(data.anim=="supershot") d_line(owner,entity,{color:"supershot"});
			else if(data.anim=="curse") d_line(owner,entity,{color:"curse"});
			else if(owner.me && !data.no_lines) { if(sd_lines) d_line(owner,entity,{color:"my_hit"}); }
	...
	socket.on('hit',function(data){
		...
		var attack_data=clone(data); delete attack_data.id; delete attack_data.hid; delete attack_data.anim;
		...
		draw_trigger(function(){
			var anim=data.anim,evade=false;
			...
			{
				var anim=G.projectiles[data.projectile||data.ray]&&G.projectiles[data.projectile||data.ray].hit_animation||data.anim;
			...
			if(entity && owner && data.damage!==undefined && !evade) // added && owner [15/01/18]
			{
				...
				if(data.anim!="curse" && !(data.damage===0 && data.reflect)) d_text("-"+data.damage,entity,{color:color});
			}
...
function update_sprite(sprite)
{
	...
	if(sprite.stype=="full") // walk
	{
		...
		else if(sprite.last_stop && mssince(sprite.last_stop)<180) i=sequence[sprite.last_walking%sequence.length];
		...
		if(sprite.s && sprite.s.charging && ms_check(sprite,"clone",80)) disappearing_clone(sprite); //!(sprite.updates%5)
	}
...
function add_monster(data)
{
	...
	var def=G.monsters[data.type],sprite=new_sprite(def.skin||data.type,"full");
	sprite.type="monster";
	...
	if(def.size) sprite.height*=def.size,sprite.width*=def.size,sprite.mscale=2,sprite.hpbar_wdisp=-5;
...
function update_filters(sprite)
{
	...
	if(sprite.pglow)
	{
		...
		if(sprite.stand || sprite.s.charging) filter.b=steps[2];
...
function cosmetics_logic(sprite)
{
	...
	needed.forEach(function(cid){
		...
		{
			var wname=cid.substr(6,99),wcx=G.items[wname].cx||{},def=G.items[wname],scale=1,extension=false,blade=false,staff=false,rod=false,bow=false,mirror=false,rotation=0;
...
function create_map()
{
	...
	map_npcs=[]; map_doors=[]; map_tiles=[]; map_entities=[]; map_animations={}; map_machines={}; water_tiles=[]; entities={};
