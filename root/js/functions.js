...
function send_target_logic()
{
	...
	if(change) socket.emit("target",{id:last_id_sent,xid:last_xid_sent});
}
...
function add_magiport(name)
{
	add_chat("^","<span style='color: white'>"+name+"</span> wants to magiport you! \
		<span class='clickable' style='color:#3E97AA' onclick='socket.emit(\"magiport\",{name:\""+name+"\"}); remove_chat(\"mp"+name+"\")'>Accept</span>",undefined,"mp"+name);
}

function add_invite(name)
{
	add_chat("^","<span style='color: white'>"+name+"</span> wants to party. \
		<span class='clickable' style='color:green' onclick='socket.emit(\"party\",{event:\"accept\",name:\""+name+"\"}); remove_chat(\"pin"+name+"\")'>Accept</span>",undefined,"pin"+name);
}
...
function add_request(name)
{
	add_chat("^","<span style='color: white'>"+name+"</span> wants to join your party. \
		<span class='clickable' style='color:#119CC1' onclick='socket.emit(\"party\",{event:\"raccept\",name:\""+name+"\"}); remove_chat(\"rq"+name+"\")'>Accept</span>",undefined,"rq"+name);
}

function add_frequest(name)
{
	add_chat("^","<span style='color: white'>"+name+"</span> wants to be your friend. \
		<span class='clickable' style='color:#DB7BB3' onclick='socket.emit(\"friend\",{event:\"accept\",name:\""+name+"\"}); remove_chat(\"frq"+name+"\")'>Accept</span>",undefined,"frq"+name);
}
...
function add_update_notes()
{
	update_notes.forEach(function(note){
		...
		if(note.indexOf("Valentine's")!=-1) color="#C987B7"; // ,color="#85C76B"
...
function show_mail_modal()
{
	get_input([
			...
			{button:"Send",onclick:function(){pcs(); socket.emit('mail',{to:$(".mrecipient").val(),subject:$(".msubject").val(),message:$(".mmsg").val()})}}
		])
}
...
function use_skill(name,target,arg)
{
	...
	{
		use("hp");
		return resolving_promise({result:"requested"});
	}
	...
	{
		if(character.rip) socket.emit('respawn');
		else socket.emit('town');
		return resolving_promise({result:"requested"});
	}
	else if(name=="cburst")
	{
		if(is_array(target))
		{
			...
			return push_deferreds("skill",target.length);
		}
		else
		{
			...
			return push_deferreds("skill",targets.length);
		}
	}
	else if(name=="3shot")
	{
...
function on_skill(key,event)
{
	...
	if(skill.type=="item")
	{
		...
		if(num>=0)
		{
			...
			else socket.emit("equip",{num:num});
		}
...
function npc_focus()
{
	...
	if(!character) return;
	map_npcs.forEach(function(element){
...
function eval_snippet()
{
	...
	if(code.search("output=")!=-1 || code.search("json_output=")!=-1) code_eval_s(code);
	else code_eval(code);
}
...
function code_eval(snippet)
{
	if(code_active)
	{
		call_code_function("eval",snippet);
	}
...
function code_eval_s(snippet)
{
	if(code_active)
	{
		call_code_function("eval_s",snippet);
	}
	else if(code_run) add_log("CODE is warming up","#DC9E48")
	else
	{
...
{
	if((amount*G.shells_to_gold)>character.gold) render_interaction("noshells");
	else
	{
		socket.emit("buy_shells",{gold:(amount*G.shells_to_gold)});
...
function buy(name,quantity)
{
	...
	var method="buy";
	if(G.items[name] && G.items[name].cash && !G.items[name].p2w && G.items[name].cash<=character.cash) method="buy_with_cash";
	socket.emit(method,{name:name,quantity:quantity});
...
function say(message,code)
{
	...
	if(message[0]=="/")
	{
		...
		else if(command=="party" || command=="invite")
		{
			var args=rest.split(" "),name=args.shift();
			var target=xtarget||ctarget;
			if(name && name.length) socket.emit('party',{event:'invite',name:name});
			else if(target && !target.me && !target.npc && target.type=="character") socket.emit('party',{event:'invite',id:target.id});
		...
		else if(command=="friend")
		{
			var args=rest.split(" "),name=args.shift();
			var target=xtarget||ctarget;
			if(name && name.length) socket.emit("friend",{event:"request",name:name});
			else if(target && !target.me && !target.npc && target.type=="character") socket.emit("friend",{event:"request",name:target.name});
	...
	else socket.emit("say",{message:message,code:code});
}
...
{
	if(!G.maps[current_map].mount) { add_log("Not in the bank.","gray"); return; }
...
{
	if(character.q.exchange) d_text("WAIT",character);
	else if(!is_code && e_item==null) d_text("INVALID",character);
	else
	{
...
function exchange_buy(token,name)
{
	...
	if(num==undefined) d_text("NO TOKENS",character);
	else
	{
...
function craft()
{
	...
	if(!j) d_text("INVALID",character);
	else socket.emit("craft",{items:items});
}
...
{
	var chest=chests[id];
	if(chest.openning && ssince(chest.openning)<5) return;
	draw_trigger(function(){
		var chest=chests[id];
...
function use(item)
{
	...
	if(!done) socket.emit("use",{item:item});
}
...
function d_text(message,x,y,args)
{
	...
	else if(color=="stun") color="#FF9601",y-=12;
	else if(color=="sugar") color="#D64770";
	else if(color=="freeze") color="#53C1FF",y-=12;
	else if(color=="burn") color="#FD9644",y-=12;
	else if(color=="crit") color="#D32D51",y-=12;
	else if(color=="sneak") color="#2D9B41",y-=12;
	else if(color=="mana") color=colors.mp;
	else if(color=="elixir") color="#E06A63";
	else if(color=="evade") color="#808B94";
	else if(color=="reflect") color="#6D62A2";
	else if(color=="supershot") color="#9B172E",y-=12;
	else if(color=="quickpunch") color="#41338B",y-=12;
	else if(color=="mentalburst") color="#4C9AE0",y-=12;
	else if(color=="burst") color="#2A8A9A",size="large";
	else if(color=="poison") color=colors.poison,size="large",y-=12;
	else if(color=="1mxp") color="#FFFFFF",fx="glow";
	else if(colors[color]) color=colors[color];
	var size=SZ[args.size]||args.size||SZ.normal;
	// console.log(size);
...
function api_call(method,args,r_args)
{
	...
	call_args={type:'POST',dataType:"json",url:base_url+api_path,data:data,success:on_success(r_args,disable),error:on_error(r_args,disable)}
	$.ajax(call_args);
	if(r_args.promise)
	{
...
setInterval(function(){
	...
	if(inside=="game" && ssince(last_servers_and_characters)>48 || inside=="game" && character && !character.afk && ssince(last_servers_and_characters)>4 || inside=="selection" && ssince(last_servers_and_characters)>4 || window.is_comm && ssince(last_servers_and_characters)>4)
	{
			...
			else if(info.purpose=="save")
			{
				...
				if(c_slot)
					...
					for(var num in ui_list)
					{
						...
						html+="<div class='gamebutton block' style='margin-bottom: -4px' onclick='load_code_s(\""+num+"\")'><span style='color: "+color+"'>["+num+"]</span> "+ui_list[num][0]+"</div>";
						one=true;
					}
