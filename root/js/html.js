...
function render_map()
{
	...
	if(current_map=="abtesting" && !events.abtesting)
	{
		events.abtesting={A:0,B:0};
		$("#topmid").append("<div id='abtesting'><div class='gamebutton' style='border-color: "+colors.A+"'>A <span class='scoreA'>0</span></div> <div class='gamebutton abtime'>5:00</div> <div class='gamebutton' style='border-color: "+colors.B+"'>B <span class='scoreB'>0</span></div></div>"); reposition_ui();
		events.abtesting.end=future_s(G.events.abtesting.duration);
	}
	else if(current_map!="abtesting" && events.abtesting)
	{
...
function render_server()
{
	...
	["wabbit","mrpumpkin","mrgreen","snowman","dragold","grinch","pinkgoo"].forEach(function(type){
		if(S[type])
		{
	...
	if(S.franky && S.franky.target)
	{
		html+=" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); emonster_click(\"franky\")'>";
		html+=sprite("franky");
		html+="<div style='color:orange; margin-top: 1px'>JOIN</div>";
		html+="</div>";
		content=true;
	}
	if(S.holidayseason)
...
function render_character_sheet()
{
	var html="<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; text-align: left' class='disableclicks'>";
		...
		html+="<div><span style='color:gray'>Fortitude:</span> "+character['for']+" <span style='color:gray'>("+parseInt((1-damage_multiplier(character['for']))*50000.0)/100.0+"%)</span></div>";
...
function render_monster(monster)
{
	...
	if(def.cooperative) html+=info_line({line:"COOPERATIVE",color:"#AEAEAE"});
...
function render_character(player)
{
	...
	html+=info_line({name:player.role&&player.role.toUpperCase()||"NAME",color:player.role&&"#E14F8B"||"gray",value:player.name,onclick:"render_cosmetics(xtarget||ctarget,{toggle:true})"});
		html+="<div class='ihtml'>";
			...
			ihtml+=info_line({name:(player.ctype=="priest"&&"HEAL"||"ATT"),color:"green",value:round(player.attack),cursed:player.s.cursed});
	...
	var bid=player.party+"|"+player.stand+"|"+(character.slots.trade1!==undefined);
		html+="<div class='bhtml'>";
			if(!player.party && character && !player.me && !player.stand)
				bhtml+=button_line({name:"PARTY",onclick:"socket.emit('party',{event:'invite',id:'"+player.id+"'})",color:"#6F3F87",pm_onclick:"cpm_window('"+(player.controller||player.name)+"')"});
			if(character && !player.me && (character.party && player.party==character.party && party_list.indexOf(character.name)<party_list.indexOf(player.name)))
				bhtml+=button_line({name:"KICK",onclick:"socket.emit('party',{event:'kick',name:'"+player.name+"'})",color:"#875045"});
			if(character && !player.me && !character.party && player.party)
				bhtml+=button_line({name:"REQUEST",onclick:"socket.emit('party',{event:'request',id:'"+player.id+"'})",color:"#6F3F87",pm_onclick:"cpm_window('"+(player.controller||player.name)+"')"});
...
function render_transports_npc()
{
	...
	if(S.duels)
	{
		for(var name in S.duels)
		{
			...
			html+="<div class='clickable' onclick='socket.emit(\"enter\",{place:\"duelland\",name:\""+duel.instance+"\"})'>&gt; Duelland <span style='color:gray'>"+duel.challenger+"</span> vs <span style='color:gray'>"+duel.vs+"</span></div>";
		}
	}
	html+="</div>";
	$("#topleftcornerui").html(html);
...
function render_mainframe()
{
	...
	var html="<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top;'>";
		...
		html+="<div class='clickable' onclick='socket.emit(\"leave\")'>logout</div>";
	html+="</div>";
	$("#topleftcornerui").html(html);
		...
		for(var j=0;j<7;j++)
		{
			...
			if(current)
			{
				...
				if(current.name=="placeholder")
				{
					...
					html=item_container({shade:G.items[name].skin,onclick:"inventory_click("+last+")",def:item,id:id,cid:cc_id,draggable:false,num:last,cnum:last,s_op:0.5,bcolor:"gray ",loader:"qplc"+rid,level:current.p&&current.p.level||undefined,iname:name});
					rids[last]=rid;
				}
				else
				{
					html=item_container({skin:skin,onclick:"inventory_click("+last+")",def:item,id:id,cid:cc_id,draggable:true,num:last,cnum:last},current);
				}
			}
			else
			{
		...
		for(var j=0;j<columns;j++)
		{
			...
			if(current)
			{
				...
				if(current.name=="placeholder")
				{
					var rid=randomStr(8);
					var name=current.p && current.p.name || "placeholder_m";
					html+=item_container({shade:G.items[name].skin,onclick:"inventory_click("+last+",event)",def:item,id:id,cid:cc_id,draggable:false,num:last,cnum:last,s_op:0.5,bcolor:"gray ",loader:"qplc"+rid,level:current.p&&current.p.level||undefined,iname:name});
					rids[last]=rid;
				}
				else
				{
					html+=item_container({skin:skin,onclick:"inventory_click("+last+",event)",def:item,id:id,cid:cc_id,draggable:true,num:last,cnum:last},current);
				}
			}
			else
			{
	...
	for(var i=0;i<4;i++)
	{
		...
		for(var j=0;j<5;j++)
		{
			...
			else if(last<items.length && items[last++])
			{
				...
				collection.push({id:id,item:item,name:current,token:token});
			}
			else
			{
				html+=item_container({size:40,draggable:false,droppable:true});
...
function render_drop(def,mult,color)
{
	...
	html+="<div style='position: relative; white-space: nowrap;'>";
		...
		if(round(def[0]*mult)>=1) html+="<div style='vertical-align: middle; display: inline-block; font-size: 24px; line-height: 50px; height: 50px; margin-left: 5px; margin-right: 8px'>"+to_pretty_num(round(def[0]*mult))+" / 1</div>";
		...
		else html+="<div style='vertical-align: middle; display: inline-block; font-size: 24px; line-height: 50px; height: 50px; margin-left: 5px; margin-right: 8px'>1 / "+to_pretty_num(round(1/(def[0]*mult)))+"</div>";
	html+="</div>";
	return html;
}
...
function render_item_help(container,name,level,pure)
{
	...
	for(var depth=0;depth<3;depth++)
	{
		...
		{
			if(in_arr(dname,names) || dname=="glitch") continue;
	...
	if(monsters.length)
	{
		...
		monsters.forEach(function(m){
			html+="<div style='display:inline-block; text-align: center' class='clickable' onclick='smart_smart_move(\"monster\",\""+m+"\")'>";
				html+="<div style='background-color:#575983; border: 2px solid #9F9FB0; display: inline-block; margin: 2px; /*"+m+"*/'>";
				if((G.monsters[m].size||1)<1) html+=sprite(G.monsters[m].skin||m,{scale:1});
				else html+=sprite(G.monsters[m].skin||m,{scale:1.5});
				html+="</div>";
...
function render_monster_info(name)
{
	...
	html+="<div class='clickable' onclick='smart_smart_move(\"monster\",\""+name+"\")'>"+sprite(G.monsters[name].skin||name,{full:true,scale:3*(G.monsters[name].size||1)})+"</div>";
...
function render_skill(selector,skill_name,args)
{
	...
	else
	{
		...
		if(skill.explanation)
		{
			...
			{
				html+=info_line({name:"CONDITION",color:"#A59FFF",value:G.conditions[skill.condition].name.toUpperCase(),onclick:"dialogs_target=xtarget||ctarget; render_condition('#topleftcornerdialog','"+skill.condition+"')"});
			}
...
function render_all_recipes()
{
	...
	object_sort(G.craft,"gold_value").forEach(function(r)
	{
		...
		recipe.items.forEach(function(i){
			...
			html+=item_container({skin:G.items[i[1]].skin,onclick:"render_item_info('"+i[1]+"')"},{name:i[1],q:i[0]});
			prev=true;
		});
...
function render_item(selector,args)
{
	...
	var prop=args.prop||calculate_item_properties(item,actual||{}),grade=calculate_item_grade(item,actual||{});
	var html="";
	...
	else
	{
		...
		if(prop.mp_cost>0) html+=bold_prop_line("A.MP Cost","+"+prop.mp_cost,colors.mp);
		else if(prop.mp_cost) html+=bold_prop_line("A.MP Cost",prop.mp_cost,colors.mp);
		if(prop.mp_reduction>0) html+=bold_prop_line("S.MP Reduction","%"+prop.mp_reduction,colors.mp);
		else if(prop.mp_reduction) html+=bold_prop_line("S.MP Increase","%"+(-prop.mp_reduction),colors.mp);
		...
		if(item.achievements) //args.monster
		{
			...
			item.achievements.forEach(function(a){
				...
				html+="<div><span style='color:"+acolor+"'>["+to_pretty_num(a[0])+"]</span> <span style='color:"+(colors[a[2]]||"gray")+"'>"+a[2].toUpperCase()+"</span> "+a[3]+"</div>";
			});
			if(args.count<100 && 0)
			{
		...
		if(args.token && args.name && args.name!=args.token)
		{
			...
			if(G.tokens[args.token][args.name]==1) phrase="TOKEN";
			if(G.tokens[args.token][args.name]<1) html+="<div><span class='clickable' style='color: "+color+"' onclick='exchange_buy(\""+args.token+"\",\""+args.name+"\")'>EXCHANGE "+(1/G.tokens[args.token][args.name])+" <span style='color:"+text+"'>[1 TOKEN]</span></span></div>";
			else html+="<div><span class='clickable' style='color: "+color+"' onclick='exchange_buy(\""+args.token+"\",\""+args.name+"\")'>EXCHANGE <span style='color:"+text+"'>["+G.tokens[args.token][args.name]+" "+phrase+"]</span></span></div>";
		}
		...
		if(!value && !args.sell && actual && !trade_item && !args.trade && !args.npc)
		{
			...
			if(item.type=="elixir" && !args.from_player)
			{
				...
				html+="<div class='clickable' onclick='socket.emit(\"equip\",{num:\""+args.num+"\"}); $(this).parent().remove()' style=\"color: #D86E89\">"+phrase+"</div>";
			}
			if((item.type=="licence" || item.type=="spawner") && !args.from_player)
			{
				html+="<div class='clickable' onclick='socket.emit(\"equip\",{num:\""+args.num+"\"}); $(this).parent().remove()' style=\"color: #574F58\">USE</div>";
			}
...
function on_rclick(current)
{
	...
	if(on) smart_eval(on);
	else if(sname!==undefined) socket.emit('unequip',{slot:sname});
	else if(snum!==undefined) socket.emit('bank',{operation:"swap",inv:-1,str:snum,pack:last_rendered_items});
	else if(inum!==undefined)
	{
		if(topleft_npc=="items")
		{
			socket.emit('bank',{operation:"swap",inv:inum,str:-1,pack:last_rendered_items});
		}
...
function on_drop(event) {
	...
	else if(slot!=undefined && inum!=undefined)
	{
		...
		else socket.emit('equip',{num:inum,slot:slot}),move=true,cache_slots[slot]=-1;
	}
...
function item_container(item,actual)
{
	...
	if(item.onclick)
	{
		if(item.draggable) container_prop+=' onclick="'+item.onclick+'" class="clickable" ';
...
function render_all_skills_and_conditions()
{
	...
	["ranger","rogue","warrior","mage","priest","paladin","merchant"].forEach(function(ctype){
		...
		object_sort(G.skills).forEach(function(s){
			...
			{
				html+=item_container({skin:skill.skin});
			}
		});
	});
	...
	object_sort(G.conditions).forEach(function(c){
		var name=c[0],condition=c[1];
		if(!condition.bad && !condition.buff && !condition.technical)
			render_cnd(name,condition);
	});
...
function render_teleporter()
{
	...
	for(var id in G.maps)
	{
		...
		{
			html+="<div class='gamebutton' style='margin-left: 5px; margin-bottom: 5px' onclick='socket.emit(\"transport\",{to:\""+id+"\"})'>"+G.maps[id].name+"</div>";
		}
	}
	html+="</div>";
		...
		html+="<div style='margin: 8px'>";
			object_sort(packs,"hpsort").forEach(function(e){
				...
				html+="<div style='display:inline-block'>";
					...
					if((G.monsters[e[0]].size||1)<1) html+=sprite(G.monsters[e[0]].skin||e[0],{scale:1});
					else html+=sprite(G.monsters[e[0]].skin||e[0],{scale:1.5});
					html+="</div>";
...
function render_interaction(type,sub_type,args)
{
	...
	else if(type=="jailer")
	{
		...
		html+="<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='socket.emit(\"leave\")'>LEAVE</div></span>";
	}
	...
	else if(type.startsWith("unlock_"))
	{
		...
		if(is_electron)
		{
			...
			html+="<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='socket.emit(\"bank\",{operation:\"unlock\",gold:1,pack:\""+pack+"\"})' style='margin-right: 5px;'>PAY "+to_pretty_num(gold)+" GOLD</div></span>";
		}
		else
		{
			...
			html+="<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='socket.emit(\"bank\",{operation:\"unlock\",gold:1,pack:\""+pack+"\"})' style='margin-right: 5px;'>USE GOLD</div><div class='slimbutton' onclick='socket.emit(\"bank\",{operation:\"unlock\",shells:1,pack:\""+pack+"\"})'>USE SHELLS</div></span>";
		}
	}
	else return;
...
function load_nearby(fallback)
{
	...
	if(someone)
	{
		...
		l.forEach(function(player)
		{
			...
			else actions+=" <span style='color: #2799DD' class='clickable' onclick='socket.emit(\"friend\",{event:\"request\",name:\""+player.name+"\"})'>+FRIEND</span>";
		...
		else
		{
			...
			info.forEach(function(player){
				...
				if(!player.party && player.name!=character.name && player.name!="Hidden")
					party="<span style='color: #34BCAF' class='clickable' onclick='parent.socket.emit(\"party\",{event:\"invite\",name:\""+player.name+"\"})'>Invite</span>";
				...
				else party="<span style='color: #9F68C0' class='clickable' onclick='parent.socket.emit(\"party\",{event:\"request\",name:\""+player.name+"\"})'>"+player.party+"</span>";
...
function sprite(name,args)
{
	try{
		...
		if(G.dimensions[name] && G.dimensions[name][3]) args.x_disp=-G.dimensions[name][3]*args.scale;
		...
		if(!(IID[name][4]%2)) html+=sprite_image(name,{cwidth:args.width,scale:args.scale,opacity:opacity,j:j,x_disp:-0.5});// old 26px width frame
		else html+=sprite_image(name,{cwidth:args.width,scale:args.scale,opacity:opacity,j:j});
...
function render_cgallery(skin,cx,slot)
{
	...
	function render_cosmetic(reset)
	{
		...
		var scale=2,found=false,onclick="onclick='if(!(xtarget||target).me) socket.emit(\"send\",{name:(xtarget||target).name,cx:\""+s[0]+"\"}); else socket.emit(\"cx\",{slot:\""+slot+"\",name:\""+s[0]+"\"}),last_cx_slot=\""+slot+"\",last_cx_name=\""+s[0]+"\",last_cx_d=[0,0]; hide_modal();' class='clickable'";
